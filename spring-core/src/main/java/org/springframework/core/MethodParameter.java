...
public class MethodParameter {
	...
	public boolean isOptional() {
		return (getParameterType() == Optional.class || hasNullableAnnotation() || (KotlinDetector.isKotlinReflectPresent() && KotlinDetector.isKotlinType(
				getContainingClass()) && KotlinDelegate.isOptional(this)));
	}
	...
	public Type getGenericParameterType() {
		...
		if (paramType == null) {
			if (this.parameterIndex < 0) {
				...
				if (this.executable instanceof Constructor && ClassUtils.isInnerClass(this.executable.getDeclaringClass())
	...
	public boolean equals(@Nullable Object other) {
		...
		return (getContainingClass() == otherParam.getContainingClass() && ObjectUtils.nullSafeEquals(this.typeIndexesPerLevel, otherParam.typeIndexesPerLevel)
				&& this.nestingLevel == otherParam.nestingLevel && this.parameterIndex == otherParam.parameterIndex && this.executable.equals(
				otherParam.executable));
	}
		...
		public static boolean isOptional(MethodParameter param) {
			...
			if (method != null && index == -1) {
				...
				if (function != null) {
					...
					KParameter parameter = parameters.stream().filter(predicate).collect(Collectors.toList()).get(index);
