...
public @interface EnableAspectJAutoProxy {
	...
	/**
	 * 表明该类采用Cglib 代理，还是JDK动态代理
	 * true 使用Cglib false 尽可能是用JDK动态代理，如果没有实现接口则使用Cglib
	...
	/**
	 * 解决内部调用不能使用代理的场景，默认为false 表示不做处理
	 * true 则表示这个代理对象的副本就可以通过AopContext.currentProxy() 获得（ThreadLocal 里面）
	 * 从而我们可以很方便在Spring框架上下文中拿到当前代理对象（处理事务时很方便）
