...
public class ContextLoader {
	...
	/**
	 * Map from (thread context) ClassLoader to corresponding（相应的） 'current' WebApplicationContext.
	 */
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		// 从ServletContext 中查找 ,是否存在以WebApplicationContext.class.getName() + ".ROOT" 为key的值
		if (servletContext.getAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE) != null) {
			// 有值 抛出异常,代表已经初始化, 在后续的代码中应该可以看到set key 的地方
		...
		try {
			...
			// 如果上下文实例 ConfigurableWebApplicationContext
			if (this.context instanceof ConfigurableWebApplicationContext) {
				// 强转
				ConfigurableWebApplicationContext cwac = (ConfigurableWebApplicationContext) this.context;
				// 容器是否刷新过, 首次进来通常这里 没有刷新过,
				if (!cwac.isActive()) {
			...
			// 将applicationContext 放入servletContext中
			// 其key为WebApplicationContext.class.getName() + ".ROOT"
