...
public class DispatcherServlet extends FrameworkServlet {
	...
	/**
	 * Detect(探测) all HandlerMappings or just expect "handlerMapping" bean?.
	 */
	...
	protected void initStrategies(ApplicationContext context) {
		...
		// 初始化handlerAdapter
		// handleAdapter 是用来调用具体的方法对用户发来的请求来进行处理
		initHandlerAdapters(context);
		// 初始化异常处理器
		// handlerExceptionResolver 是用来对请求处理过程中产生的异常进行处理
		initHandlerExceptionResolvers(context);
		// 去获取viewName
		//  requestToViewNameTranslator 用来在试图路径为空的时候,自动解析请求
		initRequestToViewNameTranslator(context);
		// 初始化视图处理器
		initViewResolvers(context);
		// flashMapManager 用来存储, 获取, 以及管理FlashMap 实例, 重定向会用到
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		// 是否检查所有的HandlesMapping 实现类并载入, 默认为true
		// <init-param>
		...
		if (this.detectAllHandlerMappings) {
			// Find all HandlerMappings in the ApplicationContext, including ancestor(祖先) contexts.
			// 查询所有HandlerMapping 在上下文,包含父级(祖先级) 上下文
			// 寻找IOC 容器中handlerMapping Bean
			Map<String, HandlerMapping> matchingBeans = BeanFactoryUtils.beansOfTypeIncludingAncestors(context, HandlerMapping.class, true, false);
			if (!matchingBeans.isEmpty()) {
				this.handlerMappings = new ArrayList<>(matchingBeans.values());
				// We keep HandlerMappings in sorted order.
				// 对HandlerMappings 进行排序
	...
	private void initHandlerExceptionResolvers(ApplicationContext context) {
		...
		if (this.detectAllHandlerExceptionResolvers) {
			...
			Map<String, HandlerExceptionResolver> matchingBeans = BeanFactoryUtils.beansOfTypeIncludingAncestors(context, HandlerExceptionResolver.class, true,
																												 false);
			if (!matchingBeans.isEmpty()) {
	...
	private void initRequestToViewNameTranslator(ApplicationContext context) {
		try {
			...
			if (logger.isTraceEnabled()) {
				logger.trace("No RequestToViewNameTranslator '" + REQUEST_TO_VIEW_NAME_TRANSLATOR_BEAN_NAME + "': using default ["
	 ...
	 * 所有的Http方法 都是被这个方法处理的, 最后要看handlerAdapters 或 handlers 它们自身去决定那个接受
	 *
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 获取用于处理当前请求的Handler
				// handler 中包含了请求URL　以及对相应的controller以及controller中的方法
				mappedHandler = getHandler(processedRequest);
				// 如果handler 为空
				if (mappedHandler == null) {
					// 未找到Handler处理, 返回Error 信息 还是抛出异常取决于属性配置 throwExceptionIfNoHandlerFound, 默认未false
					noHandlerFound(processedRequest, response);
					return;
				}

				// Determine handler adapter for the current request.
				// 如果handler 不为空,获取具体的handlerAdapter
				HandlerAdapter ha = getHandlerAdapter(mappedHandler.getHandler());

				// Process last-modified header, if supported by the handler.
				// 处理GET HEAD 请求的last-modified
				...
				// 处理成默认的视图名,也就是添加配置的prefix suffix
	 ...
	 */
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response, @Nullable HandlerExecutionChain mappedHandler,
	...
	protected HandlerAdapter getHandlerAdapter(Object handler) throws ServletException {
		if (this.handlerAdapters != null) {
			// 遍历所有的handlerAdapter, 找到和当前handler 匹配的HandlerAdapter 实例
			for (HandlerAdapter adapter : this.handlerAdapters) {
				// adapter 是否支持当前Handler
	 ...
	 */
	@Nullable
	protected ModelAndView processHandlerException(HttpServletRequest request, HttpServletResponse response, @Nullable Object handler, Exception ex) throws
			Exception {
	...
	private void triggerAfterCompletion(HttpServletRequest request, HttpServletResponse response, @Nullable HandlerExecutionChain mappedHandler,
